...
export const assignmentService = {
  async listForUser(user: { id: string; role: string }, courseId?: string) {
    if (user.role === "STUDENT") {
      return prisma.assignment.findMany({
        where: {
          lesson: courseId ? { module: { courseId } } : { module: { course: { enrollments: { some: { studentId: user.id } } } } },
        },
        include: {
    ...
    return prisma.assignment.findMany({
      where: {
        lesson: {
          module: {
            course: user.role === "ADMIN" ? (courseId ? { id: courseId } : {}) : { instructorId: user.id, ...(courseId ? { id: courseId } : {}) },
          },
        },
      },
  ...
  async create(lessonId: string, requester: { id: string; role: string }, data: {
    title: string; description: string; instructions?: string; dueDate: string; maxScore: number;
  }) {
  ...
  async update(id: string, requester: { id: string; role: string }, data: Partial<{
    ...
    const assignment = await prisma.assignment.findUnique({ where: { id }, include: { lesson: { include: { module: { include: { course: true } } } } } });
    if (!assignment) throw ApiError.notFound("Assignment not found");
